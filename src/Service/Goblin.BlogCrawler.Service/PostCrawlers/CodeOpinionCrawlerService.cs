...
{
    [ScopedDependency(ServiceType = typeof(ICrawlerService<CodeOpinionCrawlerService>))]
    public class CodeOpinionCrawlerService : Base.Service, ICrawlerService<CodeOpinionCrawlerService>
    {
        public string Name { get; } = "Code Opinion";

        public string Domain { get; } = "https://codeopinion.com";

        private readonly IGoblinRepository<SourceEntity> _sourceRepo;
        private readonly IGoblinRepository<PostEntity> _postRepo;

        public CodeOpinionCrawlerService(IGoblinUnitOfWork goblinUnitOfWork,
